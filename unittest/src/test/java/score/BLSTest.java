...
public class BLSTest extends TestBase {
    ...
    protected static class BLSTestBase {
        static protected byte[] hexToBytes(String s) {
            int len = s.length();
        ...
        static protected byte[] concatBytes(byte[]... args) {
            int length = 0;
            for (byte[] arg : args) {
                length += arg.length;
            }
            ...
            int offset = 0;
            for (byte[] arg : args) {
                System.arraycopy(arg, 0, out, offset, arg.length);
                offset += arg.length;
            }
            return out;
        }
    ...
    public static class BLSAggregateScore extends BLSTestBase {
    ...
    public static class BLSPairingScore extends BLSTestBase {
        @External
        public void test() {
            testBLS12381ecAddG1();
    ...
    @Test
    void testAggregateVerify() throws Exception {
        var score = sm.deploy(owner, BLSAggregateScore.class);
        Assertions.assertDoesNotThrow(
                () -> score.invoke(owner, "test"));
    }

    @Test
    void testPairing() throws Exception {
        var score = sm.deploy(owner, BLSPairingScore.class);
        Assertions.assertDoesNotThrow(
                () -> score.invoke(owner, "test"));
    }
}
